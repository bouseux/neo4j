                ...
                if (response) {
                    ...
                    result.data.forEach(function (value) {
                        ...
                        value.rest.forEach(function (prop, index) {
                            ...
                            } else if (prop.self && new RegExp('(http:\/\/' + options.host + ':' + options.port + '\/db\/data\/transaction\/commitdb\/data\/node\/[0-9]*)').test(prop.self)) {
                                // node result
                                newObj[result.columns[index]] = prop.data;
                            } else if (prop.self && new RegExp('(http:\/\/' + options.host + ':' + options.port + '\/db\/data\/transaction\/commitdb\/data\/relationship\/[0-9]*)').test(prop.self)) {
                                // relationship result
                                newObj[result.columns[index]] = prop.data;
                            } else {
